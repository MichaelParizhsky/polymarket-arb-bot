...
class ArbBot:
    ...
    async def run(self) -> None:
        ...
        async with PolymarketClient(self.config.polymarket, paper_trading=self.paper) as poly:
            ...
            # Live mode: sync real state from Polymarket at startup
            if not self.paper:
                await self._sync_live_state(poly, startup=True)
    ...
    async def _sync_live_state(self, poly: "PolymarketClient", startup: bool = False) -> None:
        ...
        if fills:
            ...
            for f in fills:
                tid = str(f.get("id") or f.get("trade_id") or "")
        ...
        if startup:
            ...
            if open_orders:
                logger.info(f"[LIVE] {len(open_orders)} open limit orders on CLOB")
