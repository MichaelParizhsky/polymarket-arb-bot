...
def status():
    ...
    return {
        ...
        "pnl_pct": round((p.total_pnl() / p.starting_balance) * 100, 3),
...
.card .val.green{color:#00e676}.card .val.red{color:#ff5252}.card .val.blue{color:#00e5ff}.card .val.yellow{color:#ffd740}
...
<div class="page active" id="tab-overview">
  <div class="cards">
    <div class="card"><div class="lbl">Balance</div><div class="val blue" id="balance">--</div></div>
    <div class="card"><div class="lbl">Total Value</div><div class="val" id="total-value">--</div></div>
    <div class="card"><div class="lbl">Total P&L</div><div class="val" id="pnl">--</div></div>
    <div class="card"><div class="lbl">P&L %</div><div class="val" id="pnl-pct">--</div></div>
    <div class="card"><div class="lbl">Open Positions</div><div class="val yellow" id="pos-count">--</div></div>
    <div class="card"><div class="lbl">Total Trades</div><div class="val" id="trade-count">--</div></div>
    <div class="card"><div class="lbl">Exposure</div><div class="val" id="exposure">--</div></div>
...
<div class="page" id="tab-live">
  <div class="cards" style="grid-template-columns:repeat(4,1fr);margin-bottom:14px">
    ...
    <div class="card"><div class="lbl">P&L</div><div class="val" id="live-pnl">--</div></div>
...
<div class="page" id="tab-meta">
  <div class="cards" style="grid-template-columns:repeat(3,1fr);margin-bottom:14px">
    ...
    <div class="card"><div class="lbl">Suggestions Applied</div><div class="val yellow" id="meta-applied">--</div></div>
  </div>

  <div id="meta-latest-card">
    <div class="no-data">No meta-agent analysis yet.<br><br>Run: <code>py meta_agent.py</code></div>
  </div>
...
function showTab(name){
  document.querySelectorAll('.tab').forEach((t,i)=>{t.classList.toggle('active',['overview','live','positions','trades','meta'][i]===name)});
...
const pnlChart=new Chart(pnlCtx,{type:'line',data:{labels:[],datasets:[{label:'Portfolio Value',data:[],borderColor:'#00e5ff',backgroundColor:'rgba(0,229,255,.05)',borderWidth:1.5,pointRadius:0,fill:true,tension:.3},{label:'P&L',data:[],borderColor:'#00e676',backgroundColor:'transparent',borderWidth:1,pointRadius:0,tension:.3}]},options:{...chartDefaults,plugins:{legend:{display:true,labels:{color:'#666',font:{size:10},boxWidth:10}}}}});

const stratCtx=$('stratChart').getContext('2d');
const stratChart=new Chart(stratCtx,{type:'doughnut',data:{labels:[],datasets:[{data:[],backgroundColor:['#00e676','#7986cb','#ff7043','#ffd740'],borderColor:'#0a0a0a',borderWidth:2}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'right',labels:{color:'#666',font:{size:10},boxWidth:10}}}}});
...
async function fetchAll(){
  try{
    ...
    if(currentTab==='positions'){const d=await fetch('/api/positions').then(r=>r.json());updatePositions(d);}
...
function updateStatus(s){
  ...
  const p=s.pnl||0,pp=s.pnl_pct||0;
  $('pnl').textContent=(p>=0?'+':'')+fmt(p);
  $('pnl').className='val '+(p>=0?'green':'red');
  $('pnl-pct').textContent=(pp>=0?'+':'')+pp.toFixed(3)+'%';
  $('pnl-pct').className='val '+(pp>=0?'green':'red');
  $('total-value').textContent=fmt(s.total_value);
  $('pos-count').textContent=s.open_positions;
  $('trade-count').textContent=s.total_trades;
  $('exposure').textContent=fmt(s.exposure);
  $('fees').textContent=fmt(s.fees_paid);
  // live tab
  $('live-cycle').textContent=s.cycle_count;
  $('live-uptime').textContent=s.uptime;
  $('live-pnl').textContent=(p>=0?'+':'')+fmt(p);
  $('live-pnl').className='val '+(p>=0?'green':'red');
...
function updatePositions(data){
  if(!data.length){$('positions-table').innerHTML='<div class="no-data">No open positions</div>';return;}
  $('positions-table').innerHTML=`<table><tr><th>Market</th><th>Outcome</th><th>Contracts</th><th>Avg Cost</th><th>Cost Basis</th><th>Strategy</th><th>Opened</th></tr>${data.map(p=>`<tr><td title="${p.question}">${p.question}</td><td>${p.outcome}</td><td>${p.contracts}</td><td>${fmtN(p.avg_cost)}</td><td>${fmt(p.cost_basis)}</td><td>${badge(p.strategy)}</td><td class="ts-small">${ts(p.opened_at)}</td></tr>`).join('')}</table>`;
}
...
function updateTrades(data,status){
  ...
  $('trades-table').innerHTML=`<table><tr><th>ID</th><th>Time</th><th>Strategy</th><th>Side</th><th>Contracts</th><th>Price</th><th>Amount</th><th>Notes</th></tr>${data.map(t=>`<tr><td>${t.trade_id}</td><td class="ts-small">${ts(t.timestamp)}</td><td>${badge(t.strategy)}</td><td class="${t.side.toLowerCase()}">${t.side}</td><td>${t.contracts}</td><td>${fmtN(t.price)}</td><td>${fmt(t.usdc_amount)}</td><td style="color:#555">${t.notes}</td></tr>`).join('')}</table>`;
}
...
async function fetchMeta(){
  ...
  if(hist.length){
    $('meta-history').innerHTML=`<table><tr><th>Time</th><th>Portfolio P&L</th><th>Changes Suggested</th><th>Preview</th></tr>${hist.map(h=>`<tr><td class="ts-small">${tsDate(h.timestamp)}</td><td class="${h.portfolio_pnl>=0?'buy':'sell'}">${h.portfolio_pnl>=0?'+':''}$${Number(h.portfolio_pnl).toFixed(2)}</td><td>${Object.keys(h.proposed_changes||{}).length}</td><td style="color:#555">${h.analysis_preview}</td></tr>`).join('')}</table>`;
  }else{
    $('meta-history').innerHTML='<div class="no-data">No analyses yet. Run: py meta_agent.py</div>';
  }
}
